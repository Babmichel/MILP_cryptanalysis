...
def diff_mitm_SIMON():
    ...
    with gp.Env(params=options) as env, gp.Model(env=env) as model:
        ...
        print('Do you want to generate pdf figure of th attack ? Might take 2 or minutes')
        pdf_image = int(input())
        ...
        fix_state = np.zeros([state_size, 2], dtype = object)#[round, bit index, value/differences, blue\red\purple, know/unknow]
        ...
        state_test_down_quantity = gp.quicksum(down_left_state_mid[round, bit, 2] for round, bit in product(range(MITM_down_size), range(state_size))) + gp.quicksum(down_left_state_up[round, bit, 2] for round, bit in product(range(MITM_down_size), range(state_size)))
        ...
        state_test_up_quantity = gp.quicksum(up_left_state_mid[round, bit, 2] for round, bit in product(range(MITM_up_size), range(state_size))) + gp.quicksum(up_left_state_up[round, bit, 2] for round, bit in product(range(MITM_up_size), range(state_size)))
        ...
        filtered_state_test_up = gp.quicksum(up_left_state_mid[round, bit, 2]*(1-up_AND2_equation[round, bit, 2]) for round, bit in product(range(MITM_up_size), range(state_size))) + gp.quicksum((up_left_state_up[round, bit, 2]*(1-up_AND1_equation[round, bit, 2])) for round, bit in product(range(MITM_up_size), range(state_size)))
        filtered_state_test_down = gp.quicksum(down_left_state_mid[round, bit, 2]*(1-down_AND2_equation[round, bit, 2]) for round, bit in product(range(MITM_down_size), range(state_size))) + gp.quicksum((down_left_state_up[round, bit, 2]*(1-down_AND1_equation[round, bit, 2])) for round, bit in product(range(MITM_down_size), range(state_size)))
        ...
        for round, bit in product(range(structure_size), range(state_size)):
                ...
                model.addConstr(structure_left2[round, bit, 0, 0 ,1] == gp.and_([key_structure[round, bit, 1], structure_right1[round, bit, 0, 0, 1], structure_left1[round, (bit-dec_down)%state_size, 0, 0, 1], structure_left1[round, (bit-dec_mid)%state_size, 0, 0, 1], structure_left1[round, (bit-dec_up)%state_size, 0, 0, 1]]))
                #red
                model.addConstr(structure_left1[round, bit, 0, 1, 1]==structure_right2[round, bit, 0, 1, 1])
                model.addConstr(structure_right1[round, bit, 0, 1, 1]==gp.and_([key_structure[round, bit, 2], structure_left2[round, bit, 0, 1, 1], structure_right2[round, (bit-dec_down)%state_size, 0, 1, 1], structure_right2[round, (bit-dec_mid)%state_size, 0, 1, 1], structure_right2[round, (bit-dec_up)%state_size, 0, 1, 1]]))
        ...
        for round, bit in product(range(structure_size), range(state_size)):
                #blue
                model.addConstr(structure_AND[round, bit, 1, 0, 1]==gp.and_(structure_left1[round, (bit-dec_mid)%state_size, 0, 0, 1], structure_left1[round, (bit-dec_up)%state_size, 0, 0, 1]))
                model.addConstr(structure_left2[round, bit, 1, 0, 1]==gp.and_([structure_left1[round, (bit-dec_down)%state_size, 1, 0, 1], structure_right1[round, bit, 1, 0, 1], structure_AND[round, bit, 1, 0, 1]]))
                model.addConstr(structure_right2[round, bit, 1, 0, 1]==structure_left1[round, bit, 1, 0, 1])
                #red
                model.addConstr(structure_AND[round, bit, 1, 1, 1]==gp.and_(structure_left1[round, (bit-dec_mid)%state_size, 0, 1, 1], structure_left1[round, (bit-dec_up)%state_size, 0, 1, 1]))
                model.addConstr(structure_right1[round, bit, 1, 1, 1]==gp.and_([structure_left1[round, (bit-dec_down)%state_size, 1, 1, 1], structure_left2[round, bit, 1, 1, 1], structure_AND[round, bit, 1, 1, 1]]))
        ...
        for bit in range(state_size):
                ...
                model.addConstr(fix_state[bit, 0] == gp.or_([structure_left1[round, bit, 0, 2, 1] for round in range(0, structure_size, 2)],[structure_right1[round, bit, 0, 2, 1] for round in range(1, structure_size, 2)]))    
                model.addConstr(fix_state[bit, 1] == gp.or_([structure_right1[round, bit, 0, 2, 1] for round in range(0, structure_size, 2)],[structure_left1[round, bit, 0, 2, 1] for round in range(1, structure_size, 2)]))    
        ...
        for round, bit in product(range(MITM_up_size), range(state_size)):
                ...
                model.addConstr((up_right_difference[round, bit, 1] == 1) >> (up_left_state_mid[round, (bit-dec_up)%state_size, 0] == 0), name = f"up Link state-diff <<1")
                model.addConstr((up_right_difference[round, bit, 1] == 1) >> (up_left_state_up[round, (bit-dec_mid)%state_size, 0] == 0), name = f"up Link state-diff <<8")
                model.addConstr((up_right_difference[round, bit, 2] == 1) >> (up_left_state_mid[round, (bit-dec_up)%state_size, 0] == 0), name = f"up Link state-diff <<1")
                model.addConstr((up_right_difference[round, bit, 2] == 1) >> (up_left_state_up[round, (bit-dec_mid)%state_size, 0] == 0), name = f"up Link state-diff <<8")
        ...
        for bit in range(state_size):
                model.addConstr((up_left_state_mid[0, bit, 2]==0))
                model.addConstr((up_left_state_up[0, bit, 2]==0))
                model.addConstr((up_left_state_mid[1, bit, 2]==0))
                model.addConstr((up_left_state_up[1, bit, 2]==0))
        
        #State propagation
        for round, bit in product(range(MITM_up_size), range(state_size)):
                model.addConstr((up_left_state[round, bit, 1] == 1) >> (up_left_state_down[round, bit, 1] == 1), name = f"up Link state-diff <<2")
                model.addConstr((up_left_state[round, bit, 1] == 1) >> (up_left_state_up[round, bit, 1] == 1), name = f"up Link state-diff <<8")
                model.addConstr((up_left_state[round, bit, 1] == 1) >> (up_left_state_mid[round, bit, 1] == 1), name = f"up Link state-diff <<1")

                model.addConstr((up_right_state[round, bit, 1] == 1) >> (up_left_state_mid[round, (bit-dec_mid)%state_size, 1]==1), name = f"up left state propa << 1 ")
        ...
        for round, bit in product(range(MITM_down_size), range(state_size)):
                ...
                #state test can be perform only on known differences 
                model.addConstr((down_left_difference[round, bit, 2] == 1) >> (down_left_state_mid[round, (bit-dec_mid)%state_size, 2] == 0))
                model.addConstr((down_left_difference[round, bit, 2] == 1) >> (down_left_state_up[round, (bit-dec_up)%state_size, 2] == 0))
        
        #State test not usefull on first round of key addition
        
        for bit in range(state_size):
                model.addConstr((down_left_state_mid[MITM_down_size-1, bit, 2]==0))
                model.addConstr((down_left_state_up[MITM_down_size-1, bit, 2]==0))
                model.addConstr((down_left_state_mid[MITM_down_size-2, bit, 2]==0))
                model.addConstr((down_left_state_up[MITM_down_size-2, bit, 2]==0))
        
        #State propagation
        for round, bit in product(range(MITM_down_size), range(state_size)):
                model.addConstr((down_right_state[round, bit, 1] == 1) >> (down_left_state_mid[round, bit, 1] == 1), name = f"down state : right to left <<1")
                model.addConstr((down_right_state[round, bit, 1] == 1) >> (down_left_state_down[round, bit, 1] == 1), name = f"down state : right to left <<2")
                model.addConstr((down_right_state[round, bit, 1] == 1) >> (down_left_state_up[round, bit, 1] == 1), name = f"down state : right to left <<8")

                model.addConstr((down_left_state[round, bit, 1] == 1) >> (down_left_state_mid[round, (bit-dec_mid)%state_size, 1]==1), name = f"down left state propa << 1 ")
        ...
        for round, bit in product(range(MITM_down_size-1), range(state_size)):
                #propagation to next round
                model.addConstr((down_left_state_mid[round, bit, 1] == 1) >> (down_right_state[round+1, (bit+dec_mid)%state_size, 1] == 1), name = f"down state : L=R+1 <<1")
                model.addConstr((down_left_state_down[round, bit, 1] == 1) >> (down_right_state[round+1, (bit+dec_down)%state_size, 1] == 1), name = f"down state : L=R+1 <<2")
                model.addConstr((down_left_state_up[round, bit, 1] == 1) >> (down_right_state[round+1, (bit+dec_up)%state_size, 1] == 1), name = f"down state : L=R+1 <<8")
                model.addConstr((down_right_state[round, bit, 1] == 1) >> (down_left_state[round+1, bit, 1] == 1), name = f"down state : R=L+1")
                #Key addition
                model.addConstr((down_left_state_mid[round, bit, 1] == 1) >> (key[structure_size + MITM_up_size + distinguisher_size + round, (bit+dec_mid)%state_size, 1] == 1), name = f"down Key state implication <<1")
                model.addConstr((down_left_state_up[round, bit, 1] == 1) >> (key[structure_size + MITM_up_size + distinguisher_size + round, (bit+dec_up)%state_size, 1] == 1), name = f"down Key state implication <<1")
        ...
        for round, bit, type_equation in product(range(1, MITM_up_size), range(state_size), range(3)):
                model.addConstr(up_right_equation[round, bit, type_equation] == up_left_equation[round-1, bit, type_equation])
                model.addConstr(up_left_equation[round, bit, type_equation] == up_right2_equation[round-1, bit, type_equation])
        ...
        for round, bit in product(range(MITM_down_size-1), range(state_size)):
                for type_equation in range(3):
                        model.addConstr(down_right_equation[round, bit, type_equation] == down_right2_equation[round+1, bit, type_equation])
                        model.addConstr(down_left_equation[round, bit, type_equation] == down_right_equation[round+1, bit, type_equation])
                                                        ...
                                                        if r==r_up_min:
                                                                ...
                                                                if k == 0 and r!=r_up_min:
                                                                        if up_left_state[r-structure_size-1, j*state_draw+i, 1].X == 1:
                                                                                color_right="lightcoral"
                                                                        if up_right_state[r-structure_size-1, j*state_draw+i, 1].X == 1:
                                                                                color_left="lightcoral"
                                                                if k == 1:
                                                        ...
                                                        if r==r_down_max-1:
                                                                ...
                                                                if k == 0 and r!=r_down_max-1:
                                                                        if down_right_state[r, j*state_draw+i, 1].X == 1:
                                                                                color_right="lightskyblue"
                                                                        if down_left_state[r, j*state_draw+i, 1].X == 1:
                                                                                color_left="lightskyblue"
                                                                if k == 1:
