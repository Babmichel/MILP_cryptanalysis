...
def structure(structure_size, MITM_up_size, distinguisher_size, MITM_down_size, key_size):
    ...
    with gp.Env(params=options) as env, gp.Model(env=env) as model:
        ...
        complexity = model.addVar(lb = 0.0, ub = 240.0,vtype= GRB.INTEGER, name = "complexite")
        ...
        proba_up = gp.quicksum(MITM_up_difference[round, 3, row, col, 2] for round in range(MITM_up_size) for row in range(4) for col in range(4))
        proba_down = gp.quicksum(MITM_down_difference[round, 0, row, col, 2] for round in range(MITM_down_size) for row in range(4) for col in range(4))
        ...
        count_blue = gp.quicksum(binary_bound_key_knowledge[row, col, 1, 0]*key_knowledge[row, col, 1] for row in range(4) for col in range(4)) + (key_size)*gp.quicksum(binary_bound_key_knowledge[row, col, 1, 1] for row in range(4) for col in range(4))
        count_red = gp.quicksum(binary_bound_key_knowledge[row, col, 0, 0]*key_knowledge[row, col, 0] for row in range(4) for col in range(4)) + (key_size)*gp.quicksum(binary_bound_key_knowledge[row, col, 0, 1] for row in range(4) for col in range(4))

        #complexity constraints
        
        blue_complexity = cost_differential + count_blue + proba_down + state_test_up
        red_complexity = cost_differential + count_red + proba_up + state_test_down + end_differential - start_differential
        ...
        model.addConstr(16 - fix_number <= cost_differential - start_differential + proba_up + proba_down)
        ...
        for color in range(2):
            model.addConstr(gp.quicksum(binary_bound_key_knowledge[row, col, color, 1] for row in range(4) for col in range(4)) <= 15)
        ...
        #Red are not knowing red elements at the start of the structure
        ...
        #Blue are not knowing blue elements at the end of the structure
        for row, col in product(range(4), range(4)):
        ...
        for round, row, col in product(range(MITM_up_size), range(2), range(4)):
            ...
            model.addConstr(MITM_up_state[round, 2, row, col, 0] == gp.or_(MITM_up_state[round, 1, row, col, 0], key[round + structure_size, row, col, 0], key[round + structure_size, row, col, 2])) 
            #AK is not made on the two last rows
            model.addConstr(MITM_up_state[round, 2, row+2, col, 0] == MITM_up_state[round, 1, row+2, col, 0])
        ...
        #permutation-1
        for round, row, col in product(range(MITM_up_size), range(4), range(4)):
                model.addConstr(MITM_up_difference[round, 3, row, col, 1] == MITM_up_difference[round, 2, row, (col+row)%4, 1])
        ...
        for round, row, col in product(range(MITM_up_size), range(4), range(4)):
            model.addConstr((MITM_up_difference[round, 1, row, col, 0] == 1) >> (MITM_up_difference[round, 0, row, col, 0] == 1))
            model.addConstr((MITM_up_difference[round, 1, row, col, 1] == 1) >> (MITM_up_difference[round, 0, row, col, 1] == 1))
        ...
        for round, step, row, col in product(range(distinguisher_size), range(1,4), range(4), range(4)):
            model.addConstr(distinguisher[round, step, row, col, 2] == 0)
            model.addConstr(distinguisher[round, step, row, col, 3] == 0)
        ...
        for round, row, col in product(range(distinguisher_size), range(4), range(4)):
            ...
            model.addConstr((distinguisher[round, 0, row, col, 2] == 1) >> (distinguisher[round, 1, row, col, 0] == 1))
            model.addConstr((distinguisher[round, 0, row, col, 3] == 1) >> (distinguisher[round, 1, row, col, 0] == 1))
        ...
        #link between distinguisher and propagation
        for col in range(4):
            model.addConstr((MITM_down_difference[0, 0, 0, col, 0] == gp.and_(distinguisher[distinguisher_size-1, 3, row, col, 0] for row in [0, 2, 3])))
            model.addConstr(MITM_down_difference[0, 0, 1, col, 0] == gp.and_(distinguisher[distinguisher_size-1, 3, row, col, 0] for row in [0]))
            model.addConstr(MITM_down_difference[0, 0, 2, col, 0] == gp.and_(distinguisher[distinguisher_size-1, 3, row, col, 0] for row in [1, 2]))
            model.addConstr(MITM_down_difference[0, 0, 3, col, 0] == gp.and_(distinguisher[distinguisher_size-1, 3, row, col, 0] for row in [0, 2]))

            model.addConstr((MITM_up_difference[MITM_up_size-1, 3, 0, col, 0] == gp.and_(distinguisher[0, 0, row, col, 0] for row in [1])))
            model.addConstr(MITM_up_difference[MITM_up_size-1, 3, 1, col, 0] == gp.and_(distinguisher[0, 0, row, col, 0] for row in [1, 2, 3]))
            model.addConstr(MITM_up_difference[MITM_up_size-1, 3, 2, col, 0] == gp.and_(distinguisher[0, 0, row, col, 0] for row in [1, 3]))
            model.addConstr(MITM_up_difference[MITM_up_size-1, 3, 3, col, 0] == gp.and_(distinguisher[0, 0, row, col, 0] for row in [0, 3]))
        ...
        for round, row, col in product(range(MITM_down_size), range(4), range(4)):
            model.addConstr(MITM_down_difference[round, 2, row, col, 1] == MITM_down_difference[round, 3, row, (col+row) % 4, 1])
        ...
        #SB
        for round, row, col in product(range(MITM_down_size), range(4), range(4)):
            model.addConstr((MITM_down_difference[round, 0, row, col, 0] == 1) >> (MITM_down_difference[round, 1, row, col, 0] == 1))
            model.addConstr((MITM_down_difference[round, 0, row, col, 1] == 1) >> (MITM_down_difference[round, 1, row, col, 1] == 1))
            model.addConstr((MITM_down_difference[round, 0, row, col, 2] == 1) >> (MITM_down_difference[round, 1, row, col, 0] == 1))
            model.addConstr((MITM_down_difference[round, 0, row, col, 3] == 1) >> (MITM_down_difference[round, 1, row, col, 0] == 1))
        ...
        for round, row, col in product(range(MITM_down_size), range(4), range(4)):
            for col in range(4):
                model.addConstr((MITM_down_state[round, 3, row, col, 1] == 1) >> (MITM_down_state[round, 2, row, (col-row)%4, 1] == 1))
                model.addConstr((MITM_down_state[round, 3, row, col, 0] == 1) >> (MITM_down_state[round, 2, row, (col-row)%4, 1] == 0))
        
        #key_add
        for round, row, col in product(range(MITM_down_size), range(2), range(4)):
            #state is unknow if the key is red or unknow or if the state before key addition is unknow
            model.addConstr(MITM_down_state[round, 1, row, col, 0] == gp.or_(MITM_down_state[round, 2, row, col, 0], key[round + structure_size + distinguisher_size + MITM_up_size, row, col, 0], key[round + structure_size + distinguisher_size + MITM_up_size, row, col, 1])) 
            #AK is not made on the two last rows
        ...
        if model.Status != GRB.INFEASIBLE:
            ...
            print(f"blue complexity = {blue_complexity.getValue()}")
            print(f"red complexity = {red_complexity.getValue()}")
            print(f"blue_guess = {count_blue.getValue()}")
            print(f"red_guess = {count_red.getValue()}")
            print(f"state_test_up = {state_test_up.getValue()}")
            print(f"state_test_down = {state_test_down.getValue()}")
            print(f"proba_up = {proba_up.getValue()}")
            print(f"proba_down = {proba_down.getValue()}")
            print(f"cout distingueur = {cost_differential.getValue()}")
            for row in range(4):
                for col in range(4):
                    print(f"\033[91m {key_knowledge[row, col, 0].X}", end="")
                print("")
            for row in range(4):
                for col in range(4):
                    print(f"\033[94m {key_knowledge[row, col, 1].X}", end="")
                print("")
            ...
            for step, row, col, in product(range(4), range(4), range(4)):
                ...
                for round in range(MITM_down_size):
                    if MITM_down_state[round, step, row, col, 1].X == 1:
                        left_side[round + structure_size + MITM_up_size + distinguisher_size, step, row, col] = 2
                    if MITM_down_state[round, step, row, col, 2].X == 1:
                        left_side[round + structure_size + MITM_up_size + distinguisher_size, step, row, col] = 4
            ...
            return([True, left_side, right_side, key_state])
