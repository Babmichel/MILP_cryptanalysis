...
def trun_diff_MITM(structure_round, MITM_up_round, differential_round, MITM_down_round, key_space_size):
    ...
    with gp.Env(params=options) as env, gp.Model(env=env) as model:
        ...
        model.addConstr(cost_differential >= 1) #with truncated distinguisher, 
        ...
        model.addConstr(differential_state[differential_round-1, 1, 0, 0, 1] == 0)
        model.addConstr(differential_state[differential_round-1, 1, 0, 1, 1] == 0)
        model.addConstr(differential_state[differential_round-1, 1, 0, 2, 1] == 1)
        model.addConstr(differential_state[differential_round-1, 1, 0, 3, 1] == 0)

        model.addConstr(differential_state[differential_round-1, 1, 1, 0, 1] == 0)
        model.addConstr(differential_state[differential_round-1, 1, 1, 1, 1] == 0)
        model.addConstr(differential_state[differential_round-1, 1, 1, 2, 1] == 0)
        model.addConstr(differential_state[differential_round-1, 1, 1, 3, 1] == 0)

        model.addConstr(differential_state[differential_round-1, 1, 2, 0, 1] == 0)
        model.addConstr(differential_state[differential_round-1, 1, 2, 1, 1] == 0)
        model.addConstr(differential_state[differential_round-1, 1, 2, 2, 1] == 0)
        model.addConstr(differential_state[differential_round-1, 1, 2, 3, 1] == 0)

        model.addConstr(differential_state[differential_round-1, 1, 3, 0, 1] == 0)
        model.addConstr(differential_state[differential_round-1, 1, 3, 1, 1] == 0)
        model.addConstr(differential_state[differential_round-1, 1, 3, 2, 1] == 0)
        model.addConstr(differential_state[differential_round-1, 1, 3, 3, 1] == 0)
        """
        ...
        for col in range(4):
            ...
            for row in range(4):
                model.addConstr(differential_state[differential_round-1, 0, row, col, 2] == 0)
                model.addConstr(differential_state[differential_round-1, 0, row, col, 3] == 0)
...
def recherche_attaque(mim_round, max_round, complexity):
    ...
    z=4
    for structure_round in range(4,5):
...
afficher_attaque(6,7,4,7,3,4)# structure size, MITM up size, differential distinguisher size +1, MITM down size -1, key space size, word length 

#Looking for the bests attacks on a fix number of rounds
#V,C = recherche_attaque(24,24,208) #min number of rounds, max number of rounds
