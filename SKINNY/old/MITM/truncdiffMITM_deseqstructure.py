...
def trun_diff_MITM(structure_round, MITM_up_round, differential_round, MITM_down_round, key_space_size):
    options = {
    ...
    with gp.Env(params=options) as env, gp.Model(env=env) as model:
        ...
        model.Params.timelimit = 10000.0
        ...
        complexite_rouge = model.addVar(lb = 0.0, ub = 52.0,vtype= GRB.INTEGER, name = "complexite")
        complexite_bleu = model.addVar(lb = 0.0, ub = 52.0,vtype= GRB.INTEGER, name = "complexite")
        complexite_match = model.addVar(lb = 0.0, ub = 52.0,vtype= GRB.INTEGER, name = "complexite")
        complexite = model.addVar(lb = 00.0, ub = 52.0,vtype= GRB.INTEGER, name = "complexite")
        ...
        model.setObjectiveN(complexite + complexite_bleu + complexite_rouge + complexite_match, 0, 10)
        ...
        model.addConstr(16 - end_structure_active <= cost_differential - start_differential)
        ...
        if structure_round>0:
            ...
            model.addConstr(start_structure_active == fix_quantity_structure) # we must know at the start of the structure as many word than we fix
            model.addConstr(end_structure_active == fix_quantity_structure) # we must know at the end of the structure as many word than we fix 
        ...
        #state tests are necessarly useless in the first round of MITM
        for row in range(4):
            for col in range(4):
        ...
        for round in range(2,MITM_up_round):
            for row in range(4):
                for col in range(4):
                    model.addConstr((MITM_up_state[round, 0, row, col, 2] == 1) >> (gp.quicksum(MITM_up_state[round-2, 2, row_2, col_2, 0] for row_2 in range(4) for col_2 in range(4)) >= 1))
        ...
        for round in range(MITM_up_round):
            for step in range(2):
                for row in range(4):
                    for col in range(4):
                        model.addConstr(gp.quicksum(differential_up_state[round, step, row, col, color] for color in range(4)) == 1) #state can only be unknown, blue or guess
        ...
        model.addConstr(differential_state[0, 0, 0, 3, 1] == 0)
        ...
        model.addConstr(differential_state[0, 0, 3, 0, 1] == 0)
        model.addConstr(differential_state[0, 0, 3, 1, 1] == 0)
        model.addConstr(differential_state[0, 0, 3, 2, 1] == 0)
        model.addConstr(differential_state[0, 0, 3, 3, 1] == 1)

        model.addConstr(differential_state[differential_round-1, 0, 0, 0, 1] == 0)
        model.addConstr(differential_state[differential_round-1, 0, 0, 1, 1] == 1)
        ...
        model.addConstr(differential_state[differential_round-1, 0, 2, 0, 1] == 0)
        ...
        #fixing the start and end of the truncated
        for round in range(differential_round):
            for step in range(2):
                for row in range(4):
                    for col in range(4):
        ...
        for round in range(MITM_down_round-1):
            for row in range(4):
                for col in range(4):
                    model.addConstr((MITM_down_state[round, 0, row, col, 2] == 1) >> (gp.quicksum(MITM_down_state[round+1, 0, row_2, col_2, 0] for row_2 in range(4) for col_2 in range(4)) >= 1))
        ...
        if model.Status == GRB.OPTIMAL:
            ...
            start_diff = start_differential.getValue()
            end_diff = end_differential.getValue()
            compteur_proba_up = MITM_up_guess.getValue()
            compteur_proba_down = MITM_down_guess.getValue()

            complexite_blue = blue_complexity.getValue()
            complexite_red = red_complexity.getValue()
            complexite_matching = MATCH_complexity.getValue()

            if structure_round>0:
                compteur_fix = fix_quantity_structure.getValue()
                start_structure = start_structure_active.getValue()
                end_structure = end_structure_active.getValue()
            else :
                start_structure = 16
                end_structure = 16
                compteur_fix = 16
            ...
            return(True, key_M, key_0, key_1, key_2, liste_X, liste_Y, liste_Z,Liste_X_diff, Liste_Y_diff, compteur_bleu, compteur_rouge, compteur_differentielle, start_diff, end_diff, compteur_fix, compteur_violet, compteur_statetest_haut, compteur_statetest_bas, compteur_proba_up, compteur_proba_down, start_structure, end_structure, complexite_blue, complexite_red, complexite_matching)
        else :
            return([False])

def afficher_attaque(structure, MITM_up, diff, MITM_down, key_space,z=4):
    resultat = trun_diff_MITM(structure, MITM_up, diff, MITM_down, key_space)
    if resultat[0] :
        ...
        start_structure, end_structure = resultat[21], resultat[22]
        CBleu, Crouge, Cmatch = resultat[23], resultat[24], resultat[25]
        ...
        print(structure, MITM_up, diff-1, MITM_down+1)
        ...
        print("taille etat diff")
        print(start_structure, end_structure)
        print("complexite")
        print("generation des bleus :", z*CBleu)
        print("generation des rouges :", z*Crouge)
...
def recherche_attaque(mim_round, max_round, complexity):
    ...
    for structure_round in range(2, 3):
        for MITM_up_round in range(2,10):
            for diff_round in range(2,11):
                for MITM_down_round in range(1,9):
                    if (structure_round+MITM_up_round+diff_round+MITM_down_round >= mim_round) and (structure_round+MITM_up_round+diff_round+MITM_down_round <= max_round):
                        ...
                        attaque = trun_diff_MITM(structure_round, MITM_up_round, diff_round, MITM_down_round,3)
                        if attaque[0]:
                            Valid_matrix[structure_round, MITM_up_round, diff_round, MITM_down_round] = 1
                            Complexite_matrix[structure_round, MITM_up_round, diff_round, MITM_down_round] = z*np.max([attaque[23], attaque[24], attaque[25]])
    for structure_round in range(7):
        for MITM_up_round in range(2,10):
            for diff_round in range(2,11):
                for MITM_down_round in range(2,9):
...
#for debug
#attaque = trun_diff_MITM(2, 6, 9, 6, 3, False)
#print(attaque)

#Looking for a single attack with fix parameters
afficher_attaque(2,6,10,7,3,4)# structure size, MITM up size, differential distinguisher size +1, MITM down size -1, key space size, word length 

#Looking for the bests attacks on a fix number of rounds
#V,C = recherche_attaque(24, 24, 204) #min number of rounds, max number of rounds
