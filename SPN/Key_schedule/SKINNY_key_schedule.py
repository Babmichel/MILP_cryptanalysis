...
class Model_MILP_SKINNY_key_schedule(Model_MILP_attack):
    def __init__(self, cipher_parameters, total_round, model):
        ...
        self.key_size = cipher_parameters.get('tweakey_number', 1)
    ...
    def master_key_initialisation(self):
        self.master_key = self.model.addVars(range(self.tweakey_number), 
        ...
        self.model.addConstrs(((gp.quicksum(self.master_key[tweakey, row, column, value] for value in range(3)) >=1)
                              for tweakey in range(self.tweakey_number)
                              for row in range(self.block_row_size)
                              for column in range(self.block_column_size)), 
    ...
    def keyschedule(self):
        ...
        self.model.addConstrs((self.master_key_count_guess[row, column, 1] <= 2 + self.total_round*self.master_key[tweakey, row, column, 1]
                         for tweakey in range(self.tweakey_number)
                         for row in range(self.block_row_size)
                         for column in range(self.block_column_size)), 
                         name = 'more than three guess on a same subkey bit induce master key is known')
        
        self.model.addConstrs((self.master_key_count_guess[row, column, 2] <= 2 + self.total_round*self.master_key[tweakey, row, column, 2]
                    for tweakey in range(self.tweakey_number)
                    for row in range(self.block_row_size)
                    for column in range(self.block_column_size)), 
                    name = 'more than three guess on a same subkey bit induce master key is known')
        ...
        for row in range(self.block_row_size):
            ...
            for tweakey in range(self.tweakey_number):
                ...
                for column in range(self.block_row_size):
                    if self.master_key[tweakey, row, column, 0].X == 1:
                        line += "\033[90m ■ \033[0m"
                    elif self.master_key[tweakey, row, column, 1].X == 1 and self.master_key[tweakey, row, column, 2].X == 0 :
                        line += "\033[91m ■ \033[0m"
                    elif self.master_key[tweakey, row, column, 1].X == 0 and self.master_key[tweakey, row, column, 2].X == 1 :
                        line += "\033[94m ■ \033[0m"
                    elif self.master_key[tweakey, row, column, 1].X == 1 and self.master_key[tweakey, row, column, 2].X == 1 :
                        line += "\033[95m ■ \033[0m"
                    else :
                        line+='?'
                line += '| '
