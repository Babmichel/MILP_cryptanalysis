...
with gp.Env(params=options) as env, gp.Model(env=env) as model:
    ...
    structure_round = 2
    MITM_up_round = 5
    differential_round = 15
    MITM_down_round = 5
    z=8
    probability_diff = 116
    start_diff = 0
    end_diff = 0
    ...
    objective_blue = gp.quicksum(full_key[round, key, row, col, color] for round in range(structure_round,structure_round+MITM_up_round) for key in range(key_space_size+1) for row in range(2) for col in range(4) for color in [2,3])
    objective_red = gp.quicksum(full_key[round, key, row, col, color] for round in range(structure_round + MITM_up_round,structure_round+MITM_up_round+MITM_down_round) for key in range(key_space_size+1) for row in range(2) for col in range(4) for color in [1, 3]) 
    #objective_blue = gp.quicksum(key_knowledge[row, col, 0, 1] for row in range(4) for col in range(4))
    #objective_red = gp.quicksum(key_knowledge[row, col, 0, 0] for row in range(4) for col in range(4))
    #objective_purple = gp.quicksum(full_key[round, key_index, row, col,3] for round in range(total_round-differential_round) for row in range(4) for col in range(4) for key_index in range(0,key_space_size+1))
    objective_fix_begining = gp.quicksum(structure_state[0, 0, row, col, 4] for row in range(4) for col in range(4))
    ...
    model.setObjective((objective_blue + objective_red)*mean + (objective_red-objective_blue)*(objective_red-objective_blue))
    ...
    model.addConstr(differential_up_state[MITM_up_round-1,1,0,1,1] == 1)
    ...
    model.addConstr(differential_up_state[MITM_up_round-1,1,1,0,1] == 1)
    model.addConstr(differential_up_state[MITM_up_round-1,1,1,1,1] == 1)
    model.addConstr(differential_up_state[MITM_up_round-1,1,1,2,1] == 1)
    model.addConstr(differential_up_state[MITM_up_round-1,1,1,3,1] == 1)

    model.addConstr(differential_up_state[MITM_up_round-1,1,2,0,1] == 1)
    model.addConstr(differential_up_state[MITM_up_round-1,1,2,1,1] == 1)
    ...
    model.addConstr(differential_up_state[MITM_up_round-1,1,3,0,1] == 1)
    ...
    model.addConstr(differential_down_state[0, 0, 0, 0, 1] == 1)
    model.addConstr(differential_down_state[0, 0, 0, 1, 1] == 0)
    model.addConstr(differential_down_state[0, 0, 0, 2, 1] == 0)
    model.addConstr(differential_down_state[0, 0, 0, 3, 1] == 0)

    model.addConstr(differential_down_state[0, 0, 1, 0, 1] == 0)
    ...
    model.addConstr(differential_down_state[0, 0, 2, 0, 1] == 1)
    model.addConstr(differential_down_state[0, 0, 2, 1, 1] == 0)
    model.addConstr(differential_down_state[0, 0, 2, 2, 1] == 0)
    model.addConstr(differential_down_state[0, 0, 2, 3, 1] == 1)

    model.addConstr(differential_down_state[0, 0, 3, 0, 1] == 1)
