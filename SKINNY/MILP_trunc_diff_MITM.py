        ...
        for round in range(MITM_down_round):
            for row in range(2):
                for col in range(4):
                    ...
                    model.addConstr((MITM_down_state[round, 0, row, col, 2] == 0) >> (full_key[round + structure_round + MITM_up_round + differential_round, 0, row, col, 1] + MITM_down_state[round, 1, row, col, 1] <= 1))
                    model.addConstr((MITM_down_state[round, 0, row, col, 2] == 0) >> (full_key[round + structure_round + MITM_up_round + differential_round, 0, row, col, 3] + MITM_down_state[round, 1, row, col, 1] <= 1))
