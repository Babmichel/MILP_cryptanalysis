...
with gp.Env(params=options) as env, gp.Model(env=env) as model:
    ...
    inter_key = np.zeros((nombre_tour,size_key_space, 4, 4, 3), dtype=object)
    key = np.zeros((nombre_tour, 4, 4, 3), dtype=object)
    red_key_knowledge = np.zeros((3,4,4), dtype=object)
    blue_key_knowledge = np.zeros((3,4,4), dtype=object)
    red_binary_bound = np.zeros((3,4,4,2), dtype= object)
    blue_binary_bound = np.zeros((3,4,4,2), dtype= object)
    ...
    for index in range(3):
        for row in range(4):
            for col in range(4):
                red_key_knowledge[index, row, col] = model.addVar(vtype=GRB.INTEGER, name=f'red_keyknowledge{row} {col}')
                blue_key_knowledge[index, row, col] = model.addVar(vtype=GRB.INTEGER, name=f'red_keyknowledge{row} {col}')
    
    for index in range(3):
        for row in range(4):
            for col in range(4):
                red_binary_bound[index,row,col,0] = model.addVar(vtype=GRB.BINARY, name=f'binary_bound0{index}{row}{col}')
                red_binary_bound[index,row,col,1] = model.addVar(vtype=GRB.BINARY, name=f'binary_bound1{index}{row}{col}')
                blue_binary_bound[index,row,col,0] = model.addVar(vtype=GRB.BINARY, name=f'binary_bound0{index}{row}{col}')
                blue_binary_bound[index,row,col,1] = model.addVar(vtype=GRB.BINARY, name=f'binary_bound1{index}{row}{col}')

    nombre_rouge_0=gp.quicksum(inter_key[tour,0,row,col,0] for tour in range(nombre_tour) for row in range(4) for col in range(4))
    nombre_bleu_0=gp.quicksum(inter_key[tour,0,row,col,1] for tour in range(nombre_tour) for row in range(4) for col in range(4))
    if size_key_space >=2 :
        nombre_rouge_1=gp.quicksum(inter_key[tour,1,row,col,0] for tour in range(nombre_tour) for row in range(4) for col in range(4))
        nombre_bleu_1=gp.quicksum(inter_key[tour,1,row,col,1] for tour in range(nombre_tour) for row in range(4) for col in range(4))
    else :
        nombre_rouge_1 = 0
        nombre_bleu_1 = 0
    if size_key_space >=3 :
        nombre_rouge_2=gp.quicksum(inter_key[tour,2,row,col,0] for tour in range(nombre_tour) for row in range(4) for col in range(4))
        nombre_bleu_2=gp.quicksum(inter_key[tour,2,row,col,1] for tour in range(nombre_tour) for row in range(4) for col in range(4))
    else :
        nombre_rouge_2 = 0
        nombre_bleu_2 = 0

    inconnue_total=nombre_rouge_0 + nombre_bleu_0 + nombre_rouge_1 + nombre_bleu_1 + nombre_rouge_2 + nombre_bleu_2
    model.update()
    ...
    for tour in range(nombre_tour):
        for index in range(size_key_space):
            for row in range(4):
                for col in range(4):
                    model.addConstr((inter_key[tour,index,row,col,0] == 1) >> (inter_key[tour,index,row,col,1] == 0))
                    model.addConstr((inter_key[tour,index,row,col,0] == 1) >> (inter_key[tour,index,row,col,2] == 0))
                    model.addConstr((inter_key[tour,index,row,col,1] == 1) >> (inter_key[tour,index,row,col,0] == 0))
                    model.addConstr((inter_key[tour,index,row,col,1] == 1) >> (inter_key[tour,index,row,col,2] == 0))
                    model.addConstr((inter_key[tour,index,row,col,2] == 1) >> (inter_key[tour,index,row,col,0] == 0))
                    model.addConstr((inter_key[tour,index,row,col,2] == 1) >> (inter_key[tour,index,row,col,1] == 0))
                    model.addConstr((gp.quicksum(inter_key[tour,index,row,col,i] for i in range(3)) == 1))
    
    for index in range(size_key_space):
        red_key_knowledge[index,0,0] = gp.quicksum([inter_key[0,index,0,0,0],inter_key[1,index,2,0,0],inter_key[2,index,0,2,0],inter_key[3,index,2,2,0],inter_key[4,index,1,0,0],inter_key[5,index,3,0,0],inter_key[0,index,0,0,2],inter_key[1,index,2,0,2],inter_key[2,index,0,2,2],inter_key[3,index,2,2,2],inter_key[4,index,1,0,2],inter_key[5,index,3,0,2]])
        blue_key_knowledge[index,0,0] = gp.quicksum([inter_key[0,index,0,0,1],inter_key[1,index,2,0,1],inter_key[2,index,0,2,1],inter_key[3,index,2,2,1],inter_key[4,index,1,0,1],inter_key[5,index,3,0,1],inter_key[0,index,0,0,2],inter_key[1,index,2,0,2],inter_key[2,index,0,2,2],inter_key[3,index,2,2,2],inter_key[4,index,1,0,2],inter_key[5,index,3,0,2]])
        
        red_key_knowledge[index,0,1] = gp.quicksum([inter_key[0,index,0,1,0],inter_key[1,index,2,1,0],inter_key[2,index,0,0,0],inter_key[3,index,2,0,0],inter_key[4,index,0,2,0],inter_key[5,index,2,2,0],inter_key[0,index,0,1,2],inter_key[1,index,2,1,2],inter_key[2,index,0,0,2],inter_key[3,index,2,0,2],inter_key[4,index,0,2,2],inter_key[5,index,2,2,2]])
        blue_key_knowledge[index,0,1] = gp.quicksum([inter_key[0,index,0,1,1],inter_key[1,index,2,1,1],inter_key[2,index,0,0,1],inter_key[3,index,2,0,1],inter_key[4,index,0,2,1],inter_key[5,index,2,2,1],inter_key[0,index,0,1,2],inter_key[1,index,2,1,2],inter_key[2,index,0,0,2],inter_key[3,index,2,0,2],inter_key[4,index,0,2,2],inter_key[5,index,2,2,2]])

        red_key_knowledge[index,0,2] = gp.quicksum([inter_key[0,index,0,2,0],inter_key[1,index,2,2,0],inter_key[2,index,1,0,0],inter_key[3,index,3,0,0],inter_key[4,index,1,2,0],inter_key[5,index,3,2,0],inter_key[0,index,0,2,2],inter_key[1,index,2,2,2],inter_key[2,index,1,0,2],inter_key[3,index,3,0,2],inter_key[4,index,1,2,2],inter_key[5,index,3,2,2]])
        blue_key_knowledge[index,0,2] = gp.quicksum([inter_key[0,index,0,2,1],inter_key[1,index,2,2,1],inter_key[2,index,1,0,1],inter_key[3,index,3,0,1],inter_key[4,index,1,2,1],inter_key[5,index,3,2,1],inter_key[0,index,0,2,2],inter_key[1,index,2,2,2],inter_key[2,index,1,0,2],inter_key[3,index,3,0,2],inter_key[4,index,1,2,2],inter_key[5,index,3,2,2]])
    
        red_key_knowledge[index,0,3] = gp.quicksum([inter_key[0,index,0,3,0],inter_key[1,index,2,3,0],inter_key[2,index,2,3,0],inter_key[3,index,3,3,0],inter_key[4,index,0,1,0],inter_key[5,index,2,1,0],inter_key[0,index,0,3,2],inter_key[1,index,2,3,2],inter_key[2,index,2,3,2],inter_key[3,index,3,3,2],inter_key[4,index,0,1,2],inter_key[5,index,2,1,2]])
        blue_key_knowledge[index,0,3] = gp.quicksum([inter_key[0,index,0,3,1],inter_key[1,index,2,3,1],inter_key[2,index,2,3,1],inter_key[3,index,3,3,1],inter_key[4,index,0,1,1],inter_key[5,index,2,1,1],inter_key[0,index,0,3,2],inter_key[1,index,2,3,2],inter_key[2,index,2,3,2],inter_key[3,index,3,3,2],inter_key[4,index,0,1,2],inter_key[5,index,2,1,2]])
        
        red_key_knowledge[index,1,0] = gp.quicksum([inter_key[0,index,1,0,0],inter_key[1,index,3,0,0],inter_key[2,index,1,2,0],inter_key[3,index,3,2,0],inter_key[4,index,1,1,0],inter_key[5,index,2,2,0],inter_key[0,index,1,0,2],inter_key[1,index,3,0,2],inter_key[2,index,1,2,2],inter_key[3,index,3,2,2],inter_key[4,index,1,1,2],inter_key[5,index,2,2,2]])
        blue_key_knowledge[index,1,0] = gp.quicksum([inter_key[0,index,1,0,1],inter_key[1,index,3,0,1],inter_key[2,index,1,2,1],inter_key[3,index,3,2,1],inter_key[4,index,1,1,1],inter_key[5,index,2,2,1],inter_key[0,index,1,0,2],inter_key[1,index,3,0,2],inter_key[2,index,1,2,2],inter_key[3,index,3,2,2],inter_key[4,index,1,1,2],inter_key[5,index,2,2,2]])

        red_key_knowledge[index,1,1] = gp.quicksum([inter_key[0,index,1,1,0],inter_key[1,index,3,2,0],inter_key[2,index,0,3,0],inter_key[3,index,2,3,0],inter_key[4,index,1,3,0],inter_key[5,index,3,3,0],inter_key[0,index,1,1,2],inter_key[1,index,3,2,2],inter_key[2,index,0,3,2],inter_key[3,index,2,3,2],inter_key[4,index,1,3,2],inter_key[5,index,3,3,2]])
        blue_key_knowledge[index,1,1] = gp.quicksum([inter_key[0,index,1,1,1],inter_key[1,index,3,2,1],inter_key[2,index,0,3,1],inter_key[3,index,2,3,1],inter_key[4,index,1,3,1],inter_key[5,index,3,3,1],inter_key[0,index,1,1,2],inter_key[1,index,3,2,2],inter_key[2,index,0,3,2],inter_key[3,index,2,3,2],inter_key[4,index,1,3,2],inter_key[5,index,3,3,2]])
    
        red_key_knowledge[index,1,2] = gp.quicksum([inter_key[0,index,1,2,0],inter_key[1,index,3,2,0],inter_key[2,index,1,1,0],inter_key[3,index,3,1,0],inter_key[4,index,0,3,0],inter_key[5,index,2,3,0],inter_key[0,index,1,2,2],inter_key[1,index,3,2,2],inter_key[2,index,1,1,2],inter_key[3,index,3,1,2],inter_key[4,index,0,3,2],inter_key[5,index,2,3,2]])
        blue_key_knowledge[index,1,2] = gp.quicksum([inter_key[0,index,1,2,1],inter_key[1,index,3,2,1],inter_key[2,index,1,1,1],inter_key[3,index,3,1,1],inter_key[4,index,0,3,1],inter_key[5,index,2,3,1],inter_key[0,index,1,2,2],inter_key[1,index,3,2,2],inter_key[2,index,1,1,2],inter_key[3,index,3,1,2],inter_key[4,index,0,3,2],inter_key[5,index,2,3,2]])
    
        red_key_knowledge[index,1,3] = gp.quicksum([inter_key[0,index,1,3,0],inter_key[1,index,3,3,0],inter_key[2,index,0,1,0],inter_key[3,index,2,1,0],inter_key[4,index,0,0,0],inter_key[5,index,2,0,0],inter_key[0,index,1,3,2],inter_key[1,index,3,3,2],inter_key[2,index,0,1,2],inter_key[3,index,2,1,2],inter_key[4,index,0,0,2],inter_key[5,index,2,0,2]])
        blue_key_knowledge[index,1,3] = gp.quicksum([inter_key[0,index,1,3,1],inter_key[1,index,3,3,1],inter_key[2,index,0,1,1],inter_key[3,index,2,1,1],inter_key[4,index,0,0,1],inter_key[5,index,2,0,1],inter_key[0,index,1,3,2],inter_key[1,index,3,3,2],inter_key[2,index,0,1,2],inter_key[3,index,2,1,2],inter_key[4,index,0,0,2],inter_key[5,index,2,0,2]])
        
        red_key_knowledge[index,2,0] = gp.quicksum([inter_key[0,index,2,0,0],inter_key[1,index,0,2,0],inter_key[2,index,2,2,0],inter_key[3,index,1,0,0],inter_key[4,index,3,0,0],inter_key[5,index,1,2,0],inter_key[0,index,2,0,2],inter_key[1,index,0,2,2],inter_key[2,index,2,2,2],inter_key[3,index,1,0,2],inter_key[4,index,3,0,2],inter_key[5,index,1,2,2]])
        blue_key_knowledge[index,2,0] = gp.quicksum([inter_key[0,index,2,0,1],inter_key[1,index,0,2,1],inter_key[2,index,2,2,1],inter_key[3,index,1,0,1],inter_key[4,index,3,0,1],inter_key[5,index,1,2,1],inter_key[0,index,2,0,2],inter_key[1,index,0,2,2],inter_key[2,index,2,2,2],inter_key[3,index,1,0,2],inter_key[4,index,3,0,2],inter_key[5,index,1,2,2]])
        
        red_key_knowledge[index,2,1] = gp.quicksum([inter_key[0,index,2,1,0],inter_key[1,index,0,0,0],inter_key[2,index,2,0,0],inter_key[3,index,0,2,0],inter_key[4,index,2,2,0],inter_key[5,index,1,0,0]])
        blue_key_knowledge[index,2,1] = gp.quicksum([inter_key[0,index,2,1,1],inter_key[1,index,0,0,1],inter_key[2,index,2,0,1],inter_key[3,index,0,2,1],inter_key[4,index,2,2,1],inter_key[5,index,1,0,1]])
        
        red_key_knowledge[index,2,2] = gp.quicksum([inter_key[0,index,2,2,0],inter_key[1,index,1,0,0],inter_key[2,index,3,0,0],inter_key[3,index,1,2,0],inter_key[4,index,3,2,0],inter_key[5,index,1,1,0]])
        blue_key_knowledge[index,2,2] = gp.quicksum([inter_key[0,index,2,2,1],inter_key[1,index,1,0,1],inter_key[2,index,3,0,1],inter_key[3,index,1,2,1],inter_key[4,index,3,2,1],inter_key[5,index,1,1,1]])
        
        red_key_knowledge[index,2,3] = gp.quicksum([inter_key[0,index,2,3,0],inter_key[1,index,1,3,0],inter_key[2,index,3,3,0],inter_key[3,index,0,1,0],inter_key[4,index,2,1,0],inter_key[5,index,0,0,0]])
        blue_key_knowledge[index,2,3] = gp.quicksum([inter_key[0,index,2,3,1],inter_key[1,index,1,3,1],inter_key[2,index,3,3,1],inter_key[3,index,0,1,1],inter_key[4,index,2,1,1],inter_key[5,index,0,0,1]])
        
        red_key_knowledge[index,3,0] = gp.quicksum([inter_key[0,index,3,0,0],inter_key[1,index,1,2,0],inter_key[2,index,3,2,0],inter_key[3,index,1,1,0],inter_key[4,index,3,1,0],inter_key[5,index,0,3,0]])
        blue_key_knowledge[index,3,0] = gp.quicksum([inter_key[0,index,3,0,1],inter_key[1,index,1,2,1],inter_key[2,index,3,2,1],inter_key[3,index,1,1,1],inter_key[4,index,3,1,1],inter_key[5,index,0,3,1]])
        
        red_key_knowledge[index,3,1] = gp.quicksum([inter_key[0,index,3,1,0],inter_key[1,index,0,3,0],inter_key[2,index,2,3,0],inter_key[3,index,1,3,0],inter_key[4,index,3,3,0],inter_key[5,index,0,1,0]])
        blue_key_knowledge[index,3,1] = gp.quicksum([inter_key[0,index,3,1,1],inter_key[1,index,0,3,1],inter_key[2,index,2,3,1],inter_key[3,index,1,3,1],inter_key[4,index,3,3,1],inter_key[5,index,0,1,1]])
        
        red_key_knowledge[index,3,2] = gp.quicksum([inter_key[0,index,3,2,0],inter_key[1,index,1,1,0],inter_key[2,index,3,1,0],inter_key[3,index,0,3,0],inter_key[4,index,2,3,0],inter_key[5,index,1,3,0]])
        blue_key_knowledge[index,3,2] = gp.quicksum([inter_key[0,index,3,2,1],inter_key[1,index,1,1,1],inter_key[2,index,3,1,1],inter_key[3,index,0,3,1],inter_key[4,index,2,3,1],inter_key[5,index,1,3,1]])

        red_key_knowledge[index,3,3] = gp.quicksum([inter_key[0,index,3,3,0],inter_key[1,index,0,1,0],inter_key[2,index,2,1,0],inter_key[3,index,0,0,0],inter_key[4,index,2,0,0],inter_key[5,index,0,2,0]])
        blue_key_knowledge[index,3,3] = gp.quicksum([inter_key[0,index,3,3,1],inter_key[1,index,0,1,1],inter_key[2,index,2,1,1],inter_key[3,index,0,0,1],inter_key[4,index,2,0,1],inter_key[5,index,0,2,1]])
        

    for index in range(3):
        for row in range(4):
            for col in range(4):
                model.addConstr(red_binary_bound[index, row, col, 0] + red_binary_bound[index, row, col, 1] == 1)
                model.addGenConstrIndicator(red_binary_bound[index, row, col, 0], True, red_key_knowledge[index,row,col], gp.GRB.GREATER_EQUAL, 6) 
                model.addGenConstrIndicator(red_binary_bound[index, row, col, 1], True, red_key_knowledge[index,row,col], gp.GRB.LESS_EQUAL, 2)

    for index in range(3):
        for row in range(4):
            for col in range(4):
                model.addConstr(blue_binary_bound[index, row, col, 0] + blue_binary_bound[index, row, col, 1] == 1)
                model.addGenConstrIndicator(blue_binary_bound[index, row, col, 0], True, blue_key_knowledge[index,row,col], gp.GRB.GREATER_EQUAL, 6) 
                model.addGenConstrIndicator(blue_binary_bound[index, row, col, 1], True, blue_key_knowledge[index,row,col], gp.GRB.LESS_EQUAL, 2)
    ...
    if model.Status == GRB.OPTIMAL:
        compteur_fix=0
        liste_X=[np.zeros((4,4)) for i in range(nombre_tour)]
        liste_Y=[np.zeros((4,4)) for i in range(nombre_tour)]
        liste_Z=[np.zeros((4,4)) for i in range(nombre_tour)]
        ...
        key_M=np.zeros((nombre_tour,4,4))
        for tour in range(nombre_tour):
            for row in range(4):
                for col in range(4):
        ...
        key_0=np.zeros((nombre_tour,4,4))
        for tour in range(nombre_tour):
            for row in range(4):
                for col in range(4):
        ...
        key_1=np.zeros((nombre_tour,4,4))
        if size_key_space >= 2:
            for tour in range(nombre_tour):
                for row in range(4):
                    for col in range(4):
        ...
        key_2=np.zeros((nombre_tour,4,4))
        if size_key_space == 3:
            for tour in range(nombre_tour):
                for row in range(4):
                    for col in range(4):
