...
def attack(structure_round, MITM_up_round, differential_round, MITM_down_round, key_space_size, licence = 0):
    if licence :
        options = {
    ...
    with gp.Env(params=options) as env, gp.Model(env=env) as model:
        ...
        model.setObjectiveN(-1*fix_quantity_structure, 1 , 25)
        ...
        for round in range(MITM_down_round):
            for row in range(2):
                for col in range(4):
                    ...
                    model.addConstr((MITM_down_state[round, 0, row, col, 2] == 1) >> (full_key[round + structure_round + MITM_up_round + differential_round, 0, row, col, 1] + MITM_down_state[round, 1, row, col, 1] <= 1))
                    model.addConstr((MITM_down_state[round, 0, row, col, 2] == 1) >> (full_key[round + structure_round + MITM_up_round + differential_round, 0, row, col, 3] + MITM_down_state[round, 1, row, col, 1] <= 1))
                    model.addConstr((MITM_down_state[round, 0, row, col, 0] == 1) >> (full_key[round + structure_round + MITM_up_round + differential_round, 0, row, col, 1] + MITM_down_state[round, 1, row, col, 1] <= 1))
                    model.addConstr((MITM_down_state[round, 0, row, col, 0] == 1) >> (full_key[round + structure_round + MITM_up_round + differential_round, 0, row, col, 3] + MITM_down_state[round, 1, row, col, 1] <= 1))
